	...
	case IMAGE_UTIL_GIF:
		{
			...
			for (i = _handle->current_image_count; i < _handle->image_count; i++) {
				gif_data->frames[i]->width = width;
				gif_data->frames[i]->height = height;
			}
...
int image_util_encode_set_gif_frame_delay_time(image_util_encode_h handle, unsigned long long delay_time)
{
	...
	for (i = _handle->current_image_count; i < _handle->image_count; i++)
		mm_util_gif_encode_set_frame_delay_time(gif_data->frames[i], delay_time);
...
int image_util_encode_set_input_buffer(image_util_encode_h handle, const unsigned char *src_buffer)
{
	...
	if (_handle->image_type == IMAGE_UTIL_GIF) {
		...
		if (_handle->image_count > 1) {
			image_util_retvm_if((_image_util_check_resolution(gif_data->frames[_handle->current_image_count]->width, gif_data->frames[_handle->current_image_count]->height) == false), IMAGE_UTIL_ERROR_INVALID_OPERATION, "Invalid resolution. Set resolution before setting input buffer");

	...
	case IMAGE_UTIL_GIF:
		{
			...
			if (gif_data->frames != NULL && gif_data->frames[_handle->image_count - 1] != NULL && gif_data->frames[_handle->image_count - 1]->data == NULL) {
				image_util_error("Call image_util_encode_set_input_buffer after image_util_encode_set_resolution for each frame");
				return IMAGE_UTIL_ERROR_INVALID_PARAMETER;
			}

			if (!_handle->image_count) {
			...
			gif_data->frames[_handle->image_count]->width = width;
			gif_data->frames[_handle->image_count]->height = height;
			_handle->image_count++;
...
int image_util_encode_set_input_buffer(image_util_encode_h handle, const unsigned char *src_buffer)
{
	...
	if (_handle->image_type == IMAGE_UTIL_GIF) {
		...
		_handle->src_buffer = (void *)realloc(_handle->src_buffer, (_handle->image_count) * sizeof(void *));
		_handle->src_buffer[_handle->image_count - 1] = (void *)src_buffer;

		if (!_handle->image_count || gif_data->frames == NULL || gif_data->frames[_handle->image_count - 1] == NULL) {
			image_util_error("Error allocating gif frames. Call image_util_encode_set_resolution for each frame");
			return IMAGE_UTIL_ERROR_INVALID_PARAMETER;
		}

		gif_data->frames[_handle->image_count - 1]->data = _handle->src_buffer[_handle->image_count - 1];
	} else {
		_handle->src_buffer = (void *)calloc(1, sizeof(void *));
		_handle->src_buffer[0] = (void *)src_buffer;
	}
	...
	case IMAGE_UTIL_GIF:
		{
			...
			for (i = 0; i < _handle->image_count; i++)
